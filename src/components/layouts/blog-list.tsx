    ...
    <section className="container p-6 mx-auto space-y-6 sm:space-y-12">
      <div className="grid justify-center grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {articles.map((article) => {
          ...
          const authorsBio = article.attributes.authorsBio
            ? article.attributes.authorsBio.data?.attributes
            : null;

          const avatarUrl = authorsBio
            ? getStrapiMedia(authorsBio?.avatar.data.attributes.url)
            : null;

          return (
            <Link
              href={`/${type}/${article.attributes.slug}`}
              key={article.id}
              ...
              <div className="p-6 space-y-2 relative">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xs dark:text-gray-400">
                    {moment(new Date(article.attributes.publishedAt)).fromNow()}
                  </span>
                  {authorsBio && (
                    <span className="text-xs dark:text-gray-400">
                      {authorsBio.name}
